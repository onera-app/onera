...
export function useDirectChat({
  ...
  const { data: credentials = [], isLoading: isLoadingCredentials } = useQuery({
    queryKey: ['decryptedCredentials'],
    queryFn: () => getDecryptedCredentials(token!),
    enabled: !!token && isUnlocked,
...
/**
 * Hook to invalidate credential cache
 * Use when credentials are added/removed/updated
 */
