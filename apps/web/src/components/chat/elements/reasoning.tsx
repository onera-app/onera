...
export const Reasoning = memo(
  ({
    ...
    const [internalDuration, setInternalDuration] = useState(durationProp ?? 0);
  ...
  ({ className, children, ...props }: ReasoningContentProps) => (
    ...
    >
      <div className="max-h-48 overflow-y-auto rounded-md border border-border/50 bg-muted/30 p-2.5">
        <Streamdown className="grid gap-1 text-[11px] **:text-[11px] [&_li]:my-0 [&_ol]:my-1 [&_p]:my-0 [&_ul]:my-1">
          {children}
        </Streamdown>
      </div>
    </CollapsibleContent>
